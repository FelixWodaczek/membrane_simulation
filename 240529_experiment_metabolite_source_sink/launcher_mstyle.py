...
def main():
    ...
    varied_parameters = ['langevin_seed', 'rc_membrane_metabolites', 'channel_height']
    langevin_seeds = [123]
    rcs_membrane_metabolites = np.round(np.arange(1.5, 11, 1.5), 2)
    ...
    for langevin_seed, rc_membrane_metabolites, channel_height in product(langevin_seeds, rcs_membrane_metabolites, channel_heights):
        total_simulations += 1
        
        resolution = 5
        
        sim = cg.SimLauncher_BasicGradient(langevin_seed=langevin_seed, 
        ...
        xhi_source = xhi -2  # <------------   "" # TODO: make sink sphere
        ...
        params['rc_membrane_metabolites'] = rc_membrane_metabolites # <----- this should go from 1 to 10
